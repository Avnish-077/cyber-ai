...
from report_generator import generate_investigative_brief, build_investigative_brief_pdf
    ...
    for file in files:
        ...
        try:
            ...
            if row_count == 0:
                ...
                IN_MEMORY_STORE[detected_type].extend(clean_records)
            ...
            IN_MEMORY_STORE["uploads"].append(file_meta)
...
def clear_records():
    ...
    IN_MEMORY_STORE["uploads"].clear()
    IN_MEMORY_STORE["telecom_cdr"].clear()
    IN_MEMORY_STORE["bank_upi"].clear()
    IN_MEMORY_STORE["unknown"].clear()
    IN_MEMORY_STORE["records"]["telecom_cdr"].clear()
    IN_MEMORY_STORE["records"]["bank_upi"].clear()
    IN_MEMORY_STORE["records"]["unknown"].clear()
    IN_MEMORY_STORE["latest_report"] = None
    IN_MEMORY_STORE["latest_pdf"] = None
...
def create_report():
    """
    Synthesizes current correlation + risk-scoring results into an investigative intelligence brief.
    Generates structured JSON (prime suspects, fraud clusters, immediate seizure recommendations)
    and a single-page PDF document.
    """
    correlation_data = correlate_entities(IN_MEMORY_STORE["records"])
    brief = generate_investigative_brief(correlation_data, IN_MEMORY_STORE["files"])
    pdf_bytes = build_investigative_brief_pdf(brief)

    IN_MEMORY_STORE["latest_report"] = brief
    IN_MEMORY_STORE["latest_pdf"] = pdf_bytes
...
def get_report():
    """
    Returns the latest generated investigative brief JSON, generating a fresh one if none exists.
    """
    if IN_MEMORY_STORE.get("latest_report") is None:
        return create_report()
    return IN_MEMORY_STORE["latest_report"]


@app.get("/api/report/download")
def download_report_pdf():
    """
    Returns the generated single-page PDF investigative brief as a downloadable file.
    """
    pdf_bytes = IN_MEMORY_STORE.get("latest_pdf")
    brief = IN_MEMORY_STORE.get("latest_report")

    if pdf_bytes is None or brief is None:
        brief = create_report()
        pdf_bytes = IN_MEMORY_STORE.get("latest_pdf")

    case_id = brief.get("case_id", "BRIEF").replace(" ", "_")
    filename = f"Investigative_Brief_{case_id}.pdf"
